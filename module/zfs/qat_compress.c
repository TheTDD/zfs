...
static inline void
unlock_instance(Cpa16U instanceNr)
{
	unsigned long flags;
	spin_lock_irqsave(&instance_storage_lock, flags);
...
/*******************************************************
 * Input/output buffers (static cache)
 *******************************************************/
static inline CpaStatus
...
/*******************************************************
 * ZLIB header/footer buffers (static cache)
 *******************************************************/
static inline CpaStatus
CREATE_ZLIB_BUF(Cpa8U **ptr)
{
	CpaStatus status = CPA_STATUS_FAIL;
	...
	*ptr = NULL;
	...
	if (likely(NULL != zlibCache))
	{
		void *result = kmem_cache_alloc(zlibCache, GFP_KERNEL);
		if (likely(NULL != result))
		{
			*ptr = (Cpa8U*)result;
			status = CPA_STATUS_SUCCESS;
...
#define DESTROY_ZLIB_BUF(pBuf) _destroy_zlib_buf(&(pBuf))
static inline void
_destroy_zlib_buf(Cpa8U **ptr)
{
	if (likely(NULL != *ptr))
	{
		if (likely(NULL != zlibCache))
		{
			kmem_cache_free(zlibCache, *ptr);
		}
		*ptr = NULL;
	}
}
...
/*******************************************************
 * CpaDcSessionHandle (dynamic cache)
 *******************************************************/
static inline CpaStatus
...
getReadyBufferListPtrCache(Cpa16U size)
{
	...
	/* lock for reading and check */
	read_lock_irqsave(&bufferlistptr_cache_lock, flags);
	...
	read_unlock_irqrestore(&bufferlistptr_cache_lock, flags);
...
qat_compress_init(void)
{
	...
	{
	    printk(KERN_CRIT LOG_PREFIX "failed to allocate kernel cache for buffers lists (%ld)\n", sizeof(CpaBufferList));
	    goto err;
	}
...
qat_compress_fini(void)
{
	...
	/* caches are created dynamically and need a lock */
	write_lock_irqsave(&session_cache_lock, flags);
		...
		char* dest, const int dest_len, size_t *c_len)
{
	...
	/* free space on stack by allocation of structures */
	CpaFlatBuffer *srcBuf = NULL;
	CpaFlatBuffer *destBuf = NULL;
	...
	if (likely(CPA_STATUS_SUCCESS == status))
	{
		status = CREATE_FLATBUFFER(&srcBuf);
	}

	if (likely(CPA_STATUS_SUCCESS == status))
	{
		status = CREATE_FLATBUFFER(&destBuf);
	}
	...
	if (likely(srcBuf != NULL))
	{
		DESTROY_BUFFER(srcBuf->pData);
		DESTROY_FLATBUFFER(srcBuf);
	}
	if (likely(destBuf != NULL))
	{
		DESTROY_BUFFER(destBuf->pData);
		DESTROY_FLATBUFFER(destBuf);
	}
	if (likely(headerBuf != NULL))
	{
		if (contig)
		{
		...
		const int level, const char* src, const int src_len, char* dest, const int dest_len, size_t *c_len)
{
	...
	Cpa32U sess_size = 0;
	Cpa32U ctx_size = 0;
	/* Array of pointers, 1st allocate memory for array of pointers, then for array members */
	...
	if (likely(CPA_STATUS_SUCCESS == status))
	{
		if (likely(pCap->dynamicHuffmanBufferReq))
		{
	...
	if (likely(bufferInterArray != NULL))
	{
	    for (bufferNum = 0; bufferNum < numInterBuffLists; bufferNum++)
            {
		if (likely(bufferInterArray[bufferNum] != NULL))
		{
